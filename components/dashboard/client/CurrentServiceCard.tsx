...
export default async function CurrentServiceCard() {
  ...
  const category = (Array.isArray(booking.service_categories) ? booking.service_categories[0]?.name : (booking.service_categories as any)?.name) ?? '—'
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {status === 'confirmed' ? (
            <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ backgroundColor: 'var(--primary-red)', color: '#fff' }}>
              En curso
            </span>
          ) : status === 'pending' ? (
            <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ backgroundColor: '#f3f3f3', color: 'var(--text-muted)' }}>
              Esperando proveedor
            </span>
          ) : offerCount > 0 ? (
            <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ backgroundColor: 'var(--color-success)', color: '#fff' }}>
              Oferta recibida
            </span>
          ) : (
            <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ backgroundColor: 'var(--primary-red)', color: '#fff' }}>
